...
function MediaThumb({
  ...
  return (
    <Button
      variant="ghost"
      onClick={() => onOpen(msg, url)}
      className="h-auto aspect-square overflow-hidden rounded-none p-0 hover:opacity-90"
    >
      {msg.kind === "video" ? (
    ...
    <aside className="absolute inset-0 z-40 flex min-h-0 flex-col border-l border-wa-divider bg-wa-chat md:static md:w-[400px] md:shrink-0">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <section className="flex flex-col items-center bg-wa-in px-6 py-7 text-center">
          ...
          <h3 className="mt-4 max-w-full truncate text-2xl font-normal text-wa-in-foreground">
            {chat.chatName}
          </h3>
          <p className="mt-1 text-sm text-wa-meta">
            {chat.senders.length > 2
              ? `${chat.senders.length} participants`
              : chat.senders.join(", ")}
          </p>
        </section>
